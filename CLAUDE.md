...
【不備8】P35（ビット配置グラフィック編集画面）の起動ボタンで
　　　　window.opener.location.reload()が機能しなかった
　原因：P33/P34の起動ボタン（window.open(url, '_blank', 'noopener')）
　　　をコピーしてP35の起動ボタンを実装したため、'noopener'も
　　　一緒にコピーしてしまった。'noopener'は新規ウィンドウから
　　　window.openerへの参照を意図的に断つセキュリティ上の既定挙動
　　　であり、P33/P34（参照専用・opener連携不要）では問題ないが、
　　　P35は「保存成功後にwindow.openerを再読込する」設計であるため
　　　window.openerがnullになり無言で機能しなくなっていた。画面が
　　　開くこと・保存できること自体は問題なく動作していたため、
　　　window.openerの有無を明示的に確認するまで気づけなかった。
　対策：「他の独立画面と似ているから」という理由だけで
　　　window.openのオプション（'noopener'等）を機械的にコピーしない。
　　　新規画面がopener/openee間の連携を必要とするかどうかを都度
　　　個別に判断すること。テスト観点としても「画面が開く」
　　　「操作が成功する」だけでなく、opener連携のような画面間の
　　　副作用まで踏み込んで検証すること。
```
