...
□ P21: 申請書作成
  ...
  └ チェック結果表示
  └ 一時保存・申請提出
□ P22: 申請書詳細＋承認操作（旧P23を統合・実装時に確定）
  └ 申請基本情報表示
  └ 登録ファイル一覧・ダウンロード
  └ インポート内容確認（簡易変化点表示。差分/全件切替はPhase1-4以降）
  └ チェック結果表示
  └ ステータス・承認状況表示（順番制：現在の対応順の承認者のみ操作可）
  └ 一次承認・差し戻し（ECU承認者）
  └ 二次承認・却下（LAN承認者）
  └ 判定後に[次の承認者へ回覧]で明示的に次へ進める
  └ 引き戻し操作・下書きへの[編集を続ける]導線
  └ LAN承認者による直接編集（Frame/SignalプロパティおよびFramePort/
     SignalPort編集、Excel再インポート）はPhase1-4のDB取込機能
     実装後に対応
```
