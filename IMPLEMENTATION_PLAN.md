...
### Phase2-3: ビット配置マトリクスの表示形式修正（Issue #2対応） 📝設計書反映済み・実装未着手
...
### Phase2-4: ビット配置グラフィック編集画面の新設（Issue #6対応） 📝設計書反映済み・実装未着手
...
**実装タスク（未着手）**
```
🔲 新規画面 P35（仮称）: ビット配置グラフィック編集画面
　　🔲 ルーティング追加（/#/frames/:frameId/bit-layout-editor 等、
...
🔲 動作確認
　　🔲 他申請由来のSignalがドラッグ不可であることの確認
　　🔲 二次承認者の対応順でない場合に編集不可であることの確認
　　🔲 E2E/SecOC予約領域・フレーム範囲外へのドロップ不可の確認
　　🔲 Signal間重複ドロップ時の警告表示・保存後Level2エラー表示の確認
　　🔲 保存後、元のP22タブがreloadされることの確認
```
