...
- **ビット配置グラフィック編集**: 二次審査中のLAN承認者が、自申請書が持ち込んだSignalのビット位置をドラッグ操作で調整可能
...
└── src/
    ...
    ├── pages/               # 各画面（P00〜P70、P35はP22付属の独立編集画面）
...
| P20〜P22 | 申請書管理 | 一覧・作成・詳細＋承認操作（順番制、二次審査中はFrame/Signalの直接編集も可能） |
| P30/P31 | Frame・Signal参照 | ツリー表示・サブセット別マトリクス・ビット配置表示 |
| P33/P34 | Frame/Signal詳細 | 独立ウィンドウ表示・バージョン変化点比較 |
| （P35） | ビット配置グラフィック編集 | P22の二次審査操作中のみ起動できる独立編集画面。SignalのbitPositionをドラッグで変更 |
...
※P35は設計書上は独立した画面番号ではなく、P22の付属機能として位置付けています。
...
| LAN承認者 | 二次承認・却下・in_review_2nd時編集（Frame/Signalプロパティ・FramePort/SignalPort・ビット配置の直接編集）・参照全般 |
...
| Issue | 対応内容 |
|-------|---------|
| Issue #1 | データベースの初期化機能不足 → P02プロジェクト管理画面を新設（作成/切替/リセット/削除/テーマカラー） |
| Issue #6 | FrameのSignalビット配置を直感的に調整したい → P35ビット配置グラフィック編集画面を新設（P22から起動・ドラッグでbitPositionを調整） |

対応の経緯・詳細仕様は `IMPLEMENTATION_PLAN.md`（Phase2以降）と
`docs/design/system-design-v0.6-part4-screen-design.md` のP02/P35の項を参照してください。
...
| `IMPLEMENTATION_PLAN.md` | 実装済みフェーズ（Phase1-1〜1-9）と現在進行中の磨き込みフェーズ（Phase2、Issue対応を含む）の一覧 |
