...
　└ バリアント別チェック結果レポートを申請書に添付
　└ エラーありでも申請提出可能（承認者が判断）
　└ 実行条件：物理構成取込済・バリアント定義登録済
...
| # | 出力物 | 対象データ | 備考 |
|---|-------|----------|------|
| ① | 全体通信マトリクスExcel | published | ECU単位シート・バリアント選択対応 |
| ② | ARXML | published | AUTOSAR準拠（Phase3） |
| ③ | エラーチェック結果レポート | draft/in_review | Level1・Level2別・バリアント別 |
...
| P01 | ホーム | /#/ |
| P10 | 物理構成管理 | /#/physical |
| P11 | バリアント定義 | /#/variants |
| P20 | 申請書一覧 | /#/applications |
| P21 | 申請書作成 | /#/applications/new |
| P22 | 申請書詳細 | /#/applications/{id} |
| P23 | 承認操作 | /#/applications/{id}/approve |
| P30 | 150%参照 | /#/matrix |
| P31 | バリアント別参照 | /#/matrix/variant |
| P32 | バージョン履歴 | /#/history |
| P33 | Frame詳細（独立） | /#/frames/{frameId} |
| P34 | Signal詳細（独立） | /#/signals/{signalId} |
| P40 | 断面一覧 | /#/snapshots |
| P41 | 変更履歴 | /#/changelogs |
| P50 | 出力 | /#/export |
| P60 | アクセス権管理 | /#/access-control |

### URLクエリパラメータ
```
/#/frames/{frameId}?version=01-a&variant=Gen1_HEV
/#/signals/{signalId}?version=00-a&variant=Gen1_HEV
```
...
・削除済表示トグル（P30のみ）
・バリアント選択（P31のみ）
・Frame/Signal行クリック → 画面内サイドパネルで詳細表示
・[独立画面で開く] → P33/P34へ
・バージョン履歴ボタン → P32へ
```

#### P32: バージョン履歴画面（バリアント単位）
```
・バリアント選択
・比較断面選択（断面1 ⇔ 断面2）
・変更サマリー（追加・変更・削除件数）
...
#### P33/P34: Frame/Signal詳細画面（独立）
```
・表示バージョン選択
・比較元バージョン選択（デフォルト：直前バージョン）
・プロパティ一覧（変更あり：前回値 → ★今回値）
・T/Rポート一覧
・所属Signal一覧（P33のみ）
URL: /#/frames/{frameId}?version=01-a&variant=Gen1_HEV
URL: /#/signals/{signalId}?version=01-b&variant=Gen1_HEV
```

#### P40: 断面一覧画面
```
・断面一覧（通番・確定日時・確定者）
・断面確定操作（approved→published）
...
#### P50: 出力画面
```
・全体通信マトリクスExcel出力（バリアント選択・断面選択）
・変更履歴Excel出力（比較断面選択・ECU選択）
・エラーチェック結果レポート出力（申請書選択）
・インポート雛形出力（ECU複数選択・フレーム選択・バリアント選択）
```
...
| ロール | 主な権限 | 検証フェーズ |
|-------|---------|------------|
| ECU担当者 | 申請書作成・Excel登録・引き戻し | ロール切替で再現 |
| ECU部署上司 | 一次承認・差し戻し | ロール切替で再現 |
| LAN設計マネージャー | 二次承認・却下・in_review_2nd時編集 | ロール切替で再現 |
| ネットワーク設計者 | 物理構成管理・バリアント定義・断面確定・全出力・アクセス権管理 | ロール切替で再現 |
