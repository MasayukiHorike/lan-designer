...
　ヘッダーバー・サイドバー全体の背景色に反映し、複数プロジェクトを
　切り替える際に作業中プロジェクトを視覚的に判別できるようにする
　フィールド。カラーコード（#RRGGBB形式）で保持する。
　P02では自由入力ではなく固定パレット（src/constants/
　projectTheme.ts、白文字との可読性を優先しやや暗めのトーンで
　統一）から選択させる。
