...
・フレーE2E無効でE2E利用記載 → エラー
・フレーSecOC無効でSecOC利用記載 → エラー
・途絶時間がS行に記載 → エラー
・R以外で途絶時間が記載 → エラー
...
　└ 引き戻し可能：二次承認完了前まで
　↓
【一次承認】ECU承認者（複数人・全員必須）
　└ 申請書・インポート内容・チェック結果を確認
　└ インポートExcelダウンロード可能
　└ 全員承認 → in_review_2ndへ
　└ 差し戻し → draftへ（コメント必須）
　↓
【二次承認】LAN承認者（複数人・全員必須）
　└ 申請書・インポート内容・チェック結果を確認
　└ インポートExcelダウンロード可能
　└ Excel再インポートまたは画面直接編集可能
　　 └ Frame/Signalプロパティ編集
...
・コメント入力欄
・【一次承認：ECU承認者】承認 / 差し戻し
・【二次承認：LAN承認者】承認 / 却下 / Excel再インポート / 画面直接編集
　└ Frame/Signalプロパティ編集
　└ T/Rポート編集
```
...
#### P50: 公開バージョン一覧画面
```
・断面一覧（通番・確定日時・確定者）―全ロール参照可能
・断面確定操作（approved→published）※LAN設計者のみ
...
| ロール | 主な権限 | 検証フェーズ |
|-------|---------|------------|
| ECU設計者 | 申請書作成・Excel登録・引き戻し・自担当ECU Port更新 | ロール切替で再現 |
| ECU承認者 | 一次承認・差し戻し | ロール切替で再現 |
| LAN設計者 | LAN構成管理・サブセット管理・断面確定・全出力・全ECU Port更新・アクセス権管理 | ロール切替で再現 |
| LAN承認者 | 二次承認・却下・in_review_2nd時編集 | ロール切替で再現 |
