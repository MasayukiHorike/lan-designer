...
function checkFrameGroup(
  ...
  for (const signal of signals) {
    ...
    if (signal.elementCommand && !signal.eventCondition) {
      errors.push({ code: 'EVENT_CONDITION_MISSING', message: `イベント条件が未記載です: ${sigLoc}` });
    }
    ...
    if (signal.elementCommand) {
      ...
      if (!signal.versionNo) {
        errors.push({ code: 'VERSION_MISSING', message: `バージョンNo未記載です: ${sigLoc}` });
      } else if (!isValidVersionFormat(signal.versionNo)) {
        errors.push({ code: 'VERSION_FORMAT_INVALID', message: `バージョンNoの形式が不正です: ${sigLoc}` });
      }
      ...
      if (
        ...
        if (compareVersions(signal.versionNo, existingSignal.versionNo) <= 0) {
          errors.push({ code: 'VERSION_NOT_NEWER', message: `既存より古い（または同一の）バージョンが指定されています: ${sigLoc}` });
        }
      }
    }
  }
